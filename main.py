...
for title in data:
    ...
    search_list.append(title['title'])
    title_list.append(title['title'])

for author in data:
    author_list.append(author['authors'])
    search_list.append(author['authors'])
...
def edit_pass(id):
    ...
    if request.method == 'POST' and form.validate():
        ...
        if result > 0:
            ...
            error = 'Error! Please Try after sometime'
            return render_template('home.html', error=error)
...
def issue(title):
    ...
    cur = mysql.connection.cursor()
    result1 = cur.execute("SELECT * FROM issued")
    check = cur.fetchall()
